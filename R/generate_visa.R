#' Generate VISA dataset (Visa Applications and Grants)
#'
#' @section Dataset and variable information:
#' The [Home Affairs visa statistics](https://www.homeaffairs.gov.au/research-and-statistics/statistics/visa-statistics/visa-statistics)
#' website gives information about this dataset. Use `dataset_info("VISA")` for
#' dataset information. Use `variable_info("VISA")` for variables, sources,
...
generate_visa <- function(spine = NULL, seed = 42L, output_dir = NULL,
  ...
  if (length(overseas) == 0L) {
    n_rows <- project_visa_to_parquet__(
      ...
      baseline_income  = numeric(0),
      seed             = as.integer(seed + 1800L),
    ...
    n_rows <- project_visa_to_parquet__(
      ...
      indigenous       = as.integer(spine$indigenous[overseas]),
      anzsco_code      = as.character(spine$anzsco_code[overseas]),
      baseline_income  = as.numeric(spine$baseline_income[overseas]),
      seed             = as.integer(seed + 1800L),
      out_path         = primary_path
    )
  }
