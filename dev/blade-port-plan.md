...
### Stage 5 — Orchestration consolidation + key products + build integration  _(effort M (~2-3 days). Mostly glue; the 2N key-row structure is the must-not-miss detail.)_
- **Scope:** Port .make_blade_id_bn_key (2N rows over the tsid union {key_tsid, tsid(8)}, which is {'25','20'} with the shipped metadata, not {'25','21'}), .make_blade_cn_bn_key, and .add_blade_link_reconciliation. Both tsid values and both version literals are derived from the CSVs at run time and `fplida.blade_metadata_dir` can redirect them, so R resolves them and passes them down; never hard-code them in Rust. Keep .select_key_columns in R: it enforces the keys.csv column order and is the only guard that a builder still emits every listed variable. Keep .blade_table_variable_names as it is — the table-specific column drops the earlier draft of this plan named do not exist in the code and must not be added; tests require table 1 to carry id/x_sisca06/x_anzsic93, table 4 month_actioned and table 6 cn/fn. Verify build_fplida BLADE central-stage integration (worker_results==0, stage_timings$blade set) is undisturbed. R wrapper still resolves tables and writes parquet.
- **Decided against:** a single generate_blade__ Rust entry returning a List-of-products. It would hold all 62 products alive at once — table 6 alone is 700 columns by 171,429 businesses at n=1M persons — where the R loop builds one frame, writes it and drops it. The per-product metadata the loop records (nrow/ncol/path) and the return_data path both need an R data.frame per product anyway, and the useful unit of parallelism is variables within a table, not tables. Moving the loop would also move CSV parsing into Rust, which this plan rules out.
...
- src/rust/src/blade/keys.rs — make_blade_id_bn_key (2N rows over the tsid union {key_tsid, tsid(8)}, already de-duplicated in R) and make_blade_cn_bn_key, plus the seven-value `match` frame and the two ABN-to-TAU flags derived from it. select_key_columns stays in R, applied to the returned frame. extendr: make_blade_id_bn_key__, make_blade_cn_bn_key__.
...
- `fn make_blade_person_link__(spine: List, business_spine: List, seed: i32) -> List // 28 named columns in rbind employee-then-owner order; job_number/primary_job/occupation_health_flag/birth_year/age/sex as Vec<Rint>; annual_wage/PAYG_GROSS_WAGES as Vec<f64> with NA for owners; BN/bn the identical cloned String vector, ABN_HASH_TRUNC the abn_hash_trunc() hash of it (the pre-2022 era's identifier, deliberately a different value)`
